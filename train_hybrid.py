...
parser.add_argument('-model_weight', type=str, default='./Experiments/H_model/weights.01-0.09.hdf5')
...
def train_and_predict(args):
    ...
    model.load_weights(args.model_weight,by_name=False)   #by_name=True
    print (model.summary())
    ...
    for idx in range(28,131):
        img, img_header = load(args.data + '/TrainingData/volume-' + str(idx) + '.nii' )
    ...
    model.fit_generator(generate_arrays_from_file(args.b, trainidx, img_list, tumor_list, tumorlines, liverlines,
                        ...
                        epochs= 5, verbose = 1, callbacks = [model_checkpoint,tensorboard], max_queue_size=10, workers=1, use_multiprocessing=False)
